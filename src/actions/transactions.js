...
export const fetchTransactions = () => (dispatch) => {
  return axios.get(`http://localhost:1212/api/transaction`).then(({ data }) => {
    if (data.success) {
...
export const create = (data) => (dispatch) => {
  return axios
    .post(`http://localhost:1212/api/transaction`, data)
    .then(({ data }) => {
      if (data.success) {
...
export const removeTransaction = (id) => (dispatch) => {
  return axios
    .delete(`http://localhost:1212/api/transaction/${id}`)
    .then(({ data }) => {
      if (data.success) {
        dispatch(remove(id));
        data.accounts.forEach((account) => {
