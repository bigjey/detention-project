...
class AccountForm extends Component {
  ...
  componentWillMount() {
    if (this.props.accountId && !this.props.account) {
      ...
      this.props.addFlash(
        'Could not find account with this id, redirecting to list'
      );
    }
    this.populate();
  }
  ...
  delete = (id) => {
    this.props.deleteAccount(id).then(({ success }) => {
      if (success) {
  ...
  submitHandler = (e) => {
    ...
    if (this.valid()) {
      if (account) {
        this.props
          .updateAccount(account._id, this.state.fields)
          .then(({ success, item }) => {
            if (success) {
        ...
        this.props
          .createAccount(this.state.fields)
          .then(({ success, item }) => {
            if (success) {
